...
async function main() {
  ...
  // create Users
  const alice = await prisma.user.create({ data: { name: "Alice", email: "alice@gmail.com" } });
  const ayu = await prisma.user.create({ data: { name: "Ayu", email: "ayu@gmail.com" } });
  const andini = await prisma.user.create({ data: { name: "Andini", email: "andini@gmail.com" } });
